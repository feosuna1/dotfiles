# Sources

All sources consulted while researching the PR-description spec, grouped by the
reference file that draws on them. Summaries in the sibling files are
paraphrased; consult the originals for full detail.

## Purpose / why descriptions matter
- James Croft — A Guide to Making a Good Pull Request
  https://www.jamescroft.co.uk/a-guide-to-making-a-good-pull-request/
- M. Kerem Keskin — Good Manners of a Pull Request (Delivery Hero Tech Hub)
  https://medium.com/deliveryherotechhub/good-manners-of-a-pull-request-some-best-practices-cb2de3c3aea1
- Gitmore — Pull Request Template
  https://gitmore.io/blog/pull-request-template

## Structure / template sections
- Hypertext Dispatches — How to Write an Effective GitHub PR Template
  https://tenthirtyam.org/dispatches/2026/04/04/how-to-write-an-effective-github-pull-request-template/
- Willow Voice — Write Good PR Descriptions
  https://willowvoice.com/blog/how-to-write-good-pull-request-description
- CodeAnt — Azure DevOps PR Template Examples
  https://www.codeant.ai/blogs/azure-devops-pull-request-template-examples
- luminousmen — GitHub Pull Request Templates
  https://luminousmen.com/post/github-pull-request-templates/
- Axolo — GitHub Pull Request Template
  https://axolo.co/blog/p/part-3-github-pull-request-template
- Ashlee M. Boyer — An Undefeated Pull Request Template
  https://ashleemboyer.com/blog/pull-request-template/
- minware — 10 Pull Request Template Sections That Speed Up Code Reviews
  https://www.minware.com/blog/effective-pr-template

## Writing / scoping best practices
- Atlassian — The Unwritten Pull Request Guide
  https://www.atlassian.com/blog/git/written-unwritten-guide-pull-requests
- Hugo Dias — Pull Requests Best Practices
  https://hugodias.substack.com/p/pull-requests-best-practices
- HackerOne — Writing A Great Pull Request Description
  https://www.hackerone.com/blog/writing-great-pull-request-description
- freeCodeCamp — How to Write a Pull Request Description
  https://www.freecodecamp.org/news/how-to-write-a-pull-request-description/
- Azure Repos PR Templates (oneuptime)
  https://oneuptime.com/blog/post/2026-02-16-how-to-configure-azure-repos-pull-request-templates-for-standardized-code-review-submissions/view
- Mad Devs — How to Make a Proper Description for a Pull Request
  https://maddevs.io/blog/how-to-make-a-proper-description-for-a-pull-request/
